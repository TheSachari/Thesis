...
Discrete-event simulation runner for the emergency-response dispatch environment.

This script replays an event stream (`df_pc`) made of timestamped events of two types:
- Departure events: `departure` is a dict like {1: ["VSAV"], 2: ["FPT"], ...}
- Return events: `departure == {0: "RETURN"}`

For each departure event, the simulation:
1) Iterates candidate stations in the PDD (ordered station list).
2) Selects physical vehicles that match requested functions.
3) Assigns firefighters to vehicle roles using a rule-based policy:
   - random policy by default
   - deterministic "best" policy when `--is_best` is set
4) Applies special reinforcement/lending logic for Z1 stations (Toulouse core),
   potentially inserting special departure steps (>= 99) for reinforcements.

The environment mechanics are implemented in `collective_functions.py` and imported here.

Import safety
-------------
The executable portion is wrapped in `main()` and guarded by
`if __name__ == "__main__":`, so importing this module does not start a run.

Side effects
------------
- Loads environment artifacts and event streams from disk.
- Mutates environment dictionaries in-place (vehicles, planning, durations, logs).
- Writes pickled metrics to `./Plots/<save_metrics_as>.pkl`.
"""
import pandas as pd
import numpy as np
import random
...
for row in df_pc.itertuples(index=True, name=None):
    ...
    if pdd:
        upcoming_required_skills = get_related_rows_in_time(idx, date, pdd, df_pc, n_hours)
